package com.ashu.api.crickliveinfo.Services.Implementation;

import com.ashu.api.crickliveinfo.Entites.Match;
import com.ashu.api.crickliveinfo.Repositories.MatchRepo;
...
public class MatchServiceImpl implements MatchService {
    ...
    public List<Match> getLiveMatches() {
        ...
        try {
            ...
            for (Element match : liveScoreElements) {
                ...
                match1.setBallTeamScore(bowlTeamScore);
    ...
    // this method is used to Save Matches n DB once Match is completed
    private void updateMatchInDB(Match match1) {

        //1st checking weather data is already present in DB or not??,, if not then only we'll sabe in DB
        ...
        if (match == null){
            ...
            //else condition executes when data already exist in DB

            //So 1st updating matchId of previous match with current updated match
            match1.setMatchId(match.getMatchId());

            // then updating content of previous match with this current match
            matchRepo.save(match1);
        }
